...
# Force reload wib_db, as it doesn't get reloaded otherwise, if an extension update is started from startfk
...
def restart_debug(parameter):
    ...
    if logger.getEffectiveLevel() == logging.DEBUG:
        ...
        try:
            ...
            startfk_commit_hash = os.popen(f"{git} rev-parse HEAD").read().strip()
            ...
            startfk_commit_hash = e
            image_browser_commit_hash = e
        logger.debug(f"Startfk {startfk_commit_hash}")
...
def get_image_page(img_path, page_index, filenames, keyword, sort_by, sort_order, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, exif_keyword, negative_prompt_search, use_regex, case_sensitive):
    ...
    # Set temp_dir from startfk settings, so gradio uses it
...
def show_image_info(tab_base_tag_box, num, page_index, filenames, turn_page_switch, image_gallery):
    ...
    if len(filenames) == 0:
        # This should only happen if startfk was stopped and started again and the user clicks on one of the still displayed images.
...
def on_ui_tabs():
    ...
    # Startfk's ui_loadsave uses gradio labels as keys, this does not work with image browser, as the same labels are used on different tabs
