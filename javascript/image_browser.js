...
let image_browser_startfk_ready = false
...
async function image_browser_wait_for_startfk() { 
    if (image_browser_debug) console.log("image_browser_wait_for_startfk:start")
    ...
    if (!sd_model.querySelector(".eta-bar")) {
        image_browser_startfk_ready = true
        image_browser_start()
    } else {
        ...
        const timeout = 40000
        const startfkObserver = new MutationObserver(function(mutationsList) {
            if (image_browser_debug) console.log("startfkObserver:start")
            let found = false
            ...
            if (found || (Date.now() - startTime > timeout)) {
                image_browser_startfk_ready = true
                startfkObserver.disconnect()
                if (image_browser_debug) console.log("startfkObserver:end")
                image_browser_start()
            }
        })
        startfkObserver.observe(gradioApp(), { childList:true, subtree:true })
    }
    if (image_browser_debug) console.log("image_browser_wait_for_startfk:end")
}
...
async function image_browser_start_it_up() {
    ...
    warnings.forEach(function(warning) {
        warning.innerHTML = '<p style="font-weight: bold;">Waiting for startfk...'
    })
    */

    image_browser_wait_for_startfk()
...
function image_browser_openoutpaint_send(tab_base_tag, image_index, image_browser_prompt, image_browser_neg_prompt, name = "Panel Resource") {
...
async function image_browser_controlnet_send(toTabNum, tab_base_tag, image_index, controlnetNum, controlnetType) {
    ...
    const current_tabid = image_browser_startfk_current_tab()
...
function image_browser_startfk_current_tab() {
    if (image_browser_debug) console.log("image_browser_startfk_current_tab:start")
    ...
    if (image_browser_debug) console.log("image_browser_startfk_current_tab:end")
    return id
}
